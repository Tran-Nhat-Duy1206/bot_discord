...
class PlayerService(BaseService):
    ...
    @staticmethod
    async def equip_item(guild_id: int, user_id: int, item_id: str) -> tuple[bool, str]:
    ...
    @staticmethod
    async def unequip_item(guild_id: int, user_id: int, slot: str) -> tuple[bool, str]:
    ...
    @staticmethod
    async def unlock_skill(guild_id: int, user_id: int, skill_id: str) -> SkillUnlockResult:
        skill = SKILLS.get(skill_id)
        if not skill:
            return SkillUnlockResult(ok=False, skill_id=skill_id, message="Skill không tồn tại.")
        ...
        async with BaseService.with_user_transaction(guild_id, user_id, "unlock_skill") as conn:
            ...
            if level < req:
                return SkillUnlockResult(
                    ok=False,
                    skill_id=skill_id,
                    message=f"Cần level **{req}** để unlock `{skill_id}`.",
                )

            ok = await player_repo.unlock_skill(conn, guild_id, user_id, skill_id)
            if not ok:
                return SkillUnlockResult(ok=False, skill_id=skill_id, message="Skill đã unlock trước đó.")
        ...
        return SkillUnlockResult(
            ...
            message=f"✅ Đã unlock skill **{skill.get('name', skill_id)}** (`{skill_id}`).",
        )

    @staticmethod
    async def use_skill(guild_id: int, user_id: int, skill_id: str) -> SkillUseResult:
        skill = SKILLS.get(skill_id)
        if not skill:
            return SkillUseResult(ok=False, message="Skill không tồn tại.")
        if str(skill.get("type", "")) != "active":
            return SkillUseResult(ok=False, message="Đây là passive skill, không thể dùng.")
        ...
        async with BaseService.with_user_transaction(guild_id, user_id, "use_skill") as conn:
            ...
            if skill_id == "second_wind":
                ...
                return SkillUseResult(
                    ok=True,
                    message=f"✨ {skill.get('name', 'Skill')}: hồi **{actual} HP** ({new_hp}/{max_hp})",
                )

            return SkillUseResult(ok=False, message="Skill active này chưa được implement.")
